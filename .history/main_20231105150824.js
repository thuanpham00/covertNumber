    ...
    function convertDecimalToBinary(number) {
        ...
        for (let i = 0; i < length; i++) {
            if (stringArray[i - 1] == "." && stringArray[i] != ".") {
                ...
                if (decimalPart > 0) {
                    binary.push(".");
                    let decimalPartMain = decimalPart;

                    while (decimalPartMain > 0) {
                        decimalPartMain = decimalPartMain * 2;
                        let number1 = Math.floor(decimalPartMain);
                        binary.push(number1);
                        decimalPartMain = decimalPartMain - number1;
                    }
                }
